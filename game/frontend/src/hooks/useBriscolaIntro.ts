...
export function useBriscolaIntro({
  ...
  const [briscolaIntro, setBriscolaIntro] = useState<BriscolaIntroState>({
    stage: "idle",
  });
  const [briscolaGifMeta, setBriscolaGifMeta] = useState<{
    suit: Suit;
    token: number;
  } | null>(null);
  ...
  const handleBriscolaGifPlaybackComplete = () => {
    setBriscolaIntro((previousState) =>
      previousState.stage === "gif" ? { stage: "idle" } : previousState,
    );
  };
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(() => {
      ...
      setBriscolaIntro({
        stage: "gif",
        suit,
        token: briscolaAnnouncement.eventId,
      });
    }, BRISCOLA_BANNER_MS);
