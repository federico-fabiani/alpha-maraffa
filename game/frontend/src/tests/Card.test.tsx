...
describe("Card", () => {
  ...
  it("has the correct accessible title", () => {
    ...
    expect(
      screen.getByTitle(`${RANK_FULL[3]} di ${SUIT_META.spade.label}`),
    ).toBeInTheDocument();
  });
  ...
  it("calls onClick when playable and clicked", async () => {
    ...
    render(
      <Card
        card={{ suit: "bastoni", rank: 7, playable: true }}
        onClick={onClick}
      />,
    );
  ...
  it("calls onClick when not playable (for illegal move feedback)", async () => {
    ...
    const { container } = render(
      <Card
        card={{ suit: "bastoni", rank: 7, playable: false }}
        onClick={onClick}
      />,
    );
  ...
  it("applies playable class when playable", () => {
    const { container } = render(
      <Card
        card={{ suit: "denara", rank: 2, playable: true }}
        onClick={vi.fn()}
      />,
    );
